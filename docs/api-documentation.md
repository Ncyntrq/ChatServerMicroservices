...
1. Client gọi `POST /api/auth/login` → nhận `accessToken` (TTL 2h = 7200s) và `refreshToken` (TTL 7 ngày = 604800s).
2. Mọi request tiếp theo gửi kèm header: `Authorization: Bearer <accessToken>`.
3. **Gateway** (`JwtAuthFilter`) xác thực token, giải mã `username`.
4. Gateway **inject** thêm header `X-User-Id` và `X-Username` rồi định tuyến tới service đích.
5. Các service phía sau **chỉ đọc `X-User-Id`** để xác định người dùng (tin tưởng Gateway, không tự verify JWT).

> **Quan trọng:** Các service nghiệp vụ KHÔNG nhận token trực tiếp — chúng dựa vào `X-User-Id` do Gateway gắn. Gọi thẳng vào service (bỏ qua Gateway) sẽ không có ngữ cảnh người dùng.

### 2.2. Endpoint công khai (bỏ qua xác thực)

- `/api/auth/**` — đăng ký, đăng nhập, refresh, validate.

### 2.3. Xác thực WebSocket

- Token truyền qua **query param**: `ws://localhost:8080/ws/chat?token=<jwt>`.
- Handler validate token (gọi auth-service) rồi lưu `username` vào session.

### 2.4. Phân quyền RBAC (theo server)

- Vai trò định nghĩa theo từng server (`role-service`), dùng **permission bitmask** (Owner/Admin/Moderator…).
- Service kiểm tra quyền qua FeignClient: `GET /api/servers/{serverId}/permissions/{userId}`.
...
|---|---|---|
| `Authorization: Bearer <jwt>` | Client | Token xác thực (chỉ tới Gateway) |
| `X-User-Id` | Gateway | ID/username người dùng đã xác thực |
| `X-Username` | Gateway | Username người dùng |
...
> **Cập nhật cuối:** 2026-06-16 · Tài liệu này phản ánh trạng thái code hiện tại của 12 microservices.
